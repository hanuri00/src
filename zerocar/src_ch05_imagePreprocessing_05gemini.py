...
def select_lane_lines(lines, img_shape):
    """
    허프 변환으로 검출된 직선들 중에서 실제 차선을 선택합니다.

    Args:
        lines: 허프 변환으로 검출된 직선들의 정보
        img_shape: 이미지의 크기 (높이, 너비)

    Returns:
        left_lines: 왼쪽 차선에 해당하는 직선들의 정보
        right_lines: 오른쪽 차선에 해당하는 직선들의 정보
    """
    ...
    if lines is not None:
        for line in lines:
            ...
            if -0.8 < slope < -0.4:  # 왼쪽 차선 기울기 범위 (예시)
                left_lines.append(line)
            elif 0.4 < slope < 0.8:  # 오른쪽 차선 기울기 범위 (예시)
                right_lines.append(line)

    # 2. (선택 사항) 직선들의 평균 기울기 및 절편 계산
    #     left_slope, left_intercept = calculate_average_line(left_lines)
    #     right_slope, right_intercept = calculate_average_line(right_lines)

    return left_lines, right_lines

# (선택 사항) 직선들의 평균 기울기 및 절편 계산 함수
...
def preprocess_image(image_path, output_path, img224_directory):
    try:
        ...
        lane_lines = []
        for lines_set in [left_lines, right_lines]: # 왼쪽, 오른쪽 차선에 대해 반복
